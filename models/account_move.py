...
class AccountMove(models.Model):
    ...
    def action_cancel_draft(self):
        res = super(AccountMove, self).action_cancel_draft()
        self.env['account.asset.asset'].sudo().search(
            [('invoice_id', 'in', self.ids)]).write({'active': True})
        return res
...
class AccountMoveLine(models.Model):
    ...
    @api.depends('asset_profile_id', 'move_id.invoice_date')
    def _get_asset_date(self):
        for record in self:
            record.asset_mrr = 0
            record.asset_start_date = False
            record.asset_end_date = False
            profile = record.asset_profile_id
            if profile:
                if profile.method_number == 0 or profile.method_period == 0:
                    raise UserError(_(
                        'The number of depreciations or the period length of your asset profile cannot be null.'))
                months = profile.method_number * profile.method_period
        ...
        for record in self:
            if record.asset_profile_id:
                vals = {
                    ...
                    'profile_id': record.asset_profile_id.id,
                ...
                changed_vals = record.env[
                    'account.asset.asset'].onchange_profile_id_values(
                    vals['profile_id'])
                vals.update(changed_vals['value'])
                asset = record.env['account.asset.asset'].create(vals)
                if record.asset_profile_id.open_asset:
                    asset.validate()
        return True

    @api.onchange('asset_profile_id')
    def onchange_asset_profile_id(self):
        if self.move_id.move_type == 'out_invoice' and self.asset_profile_id:
            self.account_id = self.asset_profile_id.account_asset_id.id
        elif self.move_id.move_type == 'in_invoice' and self.asset_profile_id:
            self.account_id = self.asset_profile_id.account_asset_id.id

    @api.onchange('product_uom_id')
    def _onchange_uom_id(self):
        result = super()._onchange_uom_id()
        self.onchange_asset_profile_id()
        return result
    ...
    def _onchange_product_id(self):
        ...
        if self.product_id:
            if self.move_id.move_type == 'out_invoice':
                self.asset_profile_id = self.product_id.product_tmpl_id.deferred_revenue_profile_id
            elif self.move_id.move_type == 'in_invoice':
                self.asset_profile_id = self.product_id.product_tmpl_id.asset_profile_id
        return vals

    def _set_additional_fields(self, invoice):
        if not self.asset_profile_id:
            if invoice.move_type == 'out_invoice':
                self.asset_profile_id = self.product_id.product_tmpl_id.deferred_revenue_profile_id.id
            elif invoice.move_type == 'in_invoice':
                self.asset_profile_id = self.product_id.product_tmpl_id.asset_profile_id.id
            self.onchange_asset_profile_id()
        super()._set_additional_fields(invoice)

    def get_invoice_line_account(self, type, product, fpos, company):
        return product.asset_profile_id.account_asset_id or super().get_invoice_line_account(type, product, fpos, company)
